...
def cardioGPTResponse(exercise: str, steps: int) -> list:
    ...
    for step in range(1, steps + 1):
        user_message = f"Provide step {step} of {steps} for the exercise {exercise}."
...
def gptRespone(exercise: str) -> str:
    ...
    if not AUTOMODE:
        while True:
            prompt = input(
                f"{LIGHTPURPLE}Say 'quit' to end the conversation, or continue:{CLEAR} "
            )
    ...
    for section in sections:
        ...
        for exercise in section["exercises"]:
            ...
            if STATUS_CODE == 200:
                ...
                newName = tableName(exercise["Name"], section["muscleGroup"])
